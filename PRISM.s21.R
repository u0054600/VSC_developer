...
# and for the non restarters we setwd db directory to hace the /lustre1/project/stg_00080/vrc/Projects/Project_Microbiome/DB/DB_nt/taxonomy4blast.sqlite3 file 
#in v20 we have looaded xgboost,  still missing the very last step.
# v21 fixed a waring group_indices() at line 1121
setwd("/lustre1/project/stg_00080/vrc/Projects/Project_Microbiome/DB/DB_nt/")
...
library("xgboost",  lib="/staging/leuven/stg_00080/vrc/Projects/Project_Rushuang/Rpack")
...
saveRDS(opt, file=paste0(data_path,"opts.rds"))
cat(paste('\nSample name:', sample, '\n'))
...
pipeline_path="/staging/leuven/stg_00080/vrc/Projects/Project_Rushuang/scbac_functions/"
str = paste0( 
             'cut -f1-3,6-8 ', 
...
system(paste0("rm ",out_path, sample, '.kraken.microbiome.output.txt'))
...
print.noquote("ready for string line 161")
print.noquote(str)
...
print.noquote("ready for string line 202")

#quit("no")

### subsample
cat(paste('Started subsampling line 204 at', Sys.time(), '\n'))
...
# we modified the block above into the line below
header_taxid = gsub('.*taxid\\|', '', headers)
...
cat(paste('Started STAR line 313 at', Sys.time(), '\n'))
if(paired == T){
...
save.image(paste0(sample,"_saveImageline_360.Rdata"))
# all works till here
# blast
cat(paste('Started species Blast at', Sys.time(), '\n'))
system (paste0("module load  SQLite"))
...
print.noquote(str)
system(paste0( "echo -e ",  str, " >  ", out_path , sample, '_temp_command_BLAST')) 
system(str) 
}
...
save.image(paste0(sample,"_saveImageline_403.Rdata"))
...
cat(paste('Started multimapping filtering analysis', Sys.time(), '\n'))
...
cat(paste('Started specific assignment', Sys.time(), '\n'))
print.noquote( "start path #2 to replace multicore @ line 439")
...
LISTdf <- list()
for (i in 1:nrow(x)){
LISTdf[[i]] <-  mpa[which(kr2$V7 %in% colnames(x)[which(x[i,]> 0)]), 'V1'] %>% as.character() %>%
    strsplit('\\|') %>%
    unlist() %>%
    table() %>%
    as.data.frame() %>%
...
df <- do.call(rbind, LISTdf) %>% tibble()


print.noquote("end of patch #2 line 492")
...
# we are in a while loop but I dd the patch here again.  
rm (df)
  dfLIST <- list()
  
  for (i in 1:nrow(x)){
  dfLIST[[i]] <-    mpa[which(kr2$V7 %in% colnames(x)[which(x[i,]> 0)]), 'V1'] %>% as.character() %>% 
     strsplit('\\|') %>% 
      unlist() %>% 
     table() %>% 
      as.data.frame() %>%
...
save.image(paste0(sample,"_saveImageline_611.Rdata"))
...
print.noquote("similar as previous path, with furture multicore, replaced by loop @ line 621") 
...
## Actual patch below didn't work since x for  sc5rALBR003_hg19 was empty 
LISTdf2 <- list()
	for (i in 1:nrow(x)){
	LISTdf2[[i]] <- mpa[which(kr2$V7 %in% colnames(x)[which(x[i,]> 0)]), 'V1'] %>% as.character() %>% 
    strsplit('\\|') %>% 
    unlist() %>% 
    table() %>% 
    as.data.frame() %>%
...
df2 <- do.call(rbind, LISTdf2)

print("end of patch line 667")
save.image(paste0(out_path, sample,"line667.R"))
...
save.image(paste0(sample,"_saveImageline_750.Rdata"))
...
print.noquote("after the future map on line 750")
...
cat(paste('Started multiblast analysis atline 907', Sys.time(), '\n'))
...
# we modified the block above into the line below # Adapted again header_ids ipv header_taxid on 29 April in v20
header_ids= gsub('.*taxid\\|', '', headers) 
...
# we modified the block above into the line below
header_ids2 = gsub('.*taxid\\|', '', headers2)
  ShortRead::writeFasta(fa2_new, file = paste0(out_path, sample, '-new_2.fa'))
}


save.image(paste0(out_path, sample, ".image.line 1051.R"))
...
save.image(paste0(out_path, sample, ".image.line 1088.R"))
...
#x = x %>% mutate(duplicate_group = group_indices(x, across(everything()))) # thi was the original line which we replaced by the following 4 lines.
x <- x %>%
  group_by(across(everything())) %>%
  mutate(duplicate_group = group_indices()) %>%
#  ungroup() # It's often good practice to ungroup afterwards, # waas suggested online but deactivate the line
z = distinct(x)


save.image(paste0(out_path, sample, ".image.line 1100v21.R"))
##future::plan(multisession)
